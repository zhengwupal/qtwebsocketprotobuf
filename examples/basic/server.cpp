...
int main(int argc, char* argv[])
{
    ...
    server.setSessionConnectedCallback([&server](quint64 sessionId) {
    ...
    server.setSessionDisconnectedCallback([](quint64 sessionId, const QString& reason) {
        LOG_INFO(QString("Session %1 disconnected: %2").arg(sessionId).arg(reason));
    });

    processor->setEchoCallback([&server](const example::EchoMessage& message, quint64 sessionId, quint32 messageId) {
        if (sessionId == 0) {
