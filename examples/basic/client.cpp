...
int main(int argc, char* argv[])
{
    ...
    QString sessionIdStr = parser.value(sessionIdOption);
    quint64 sessionId = sessionIdStr.isEmpty() ? 0 : sessionIdStr.toULongLong();
    ...
    QObject::connect(processor.data(), &ExampleMessageProcessor::connAckReceived,
                     [&client, sessionId](quint64 receivedSessionId) {
                         client.setSessionId(receivedSessionId);
                         if (sessionId != 0 && sessionId == receivedSessionId) {
                             LOG_INFO(QString("Session successfully restored: %1").arg(receivedSessionId));
                         } else if (sessionId != 0 && sessionId != receivedSessionId) {
    ...
    processor->setEchoCallback([](const example::EchoMessage& message, quint64 /*sessionId*/, quint32 /*messageId*/) {
