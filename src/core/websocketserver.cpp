...
void WebSocketServer::stop()
{
    ...
    const QList<quint64> sessionIds = d->sessionManager->getAllSessionIds();
    for (const quint64& sessionId : sessionIds) {
...
void WebSocketServer::onNewConnection()
{
    ...
    if (requestUrl.hasQuery()) {
        ...
        if (!requestedSessionId.isEmpty()) {
            quint64 requestedId = requestedSessionId.toULongLong();
            if (d->sessionManager->restoreSession(socket, requestedId)) {
...
void WebSocketServer::onSocketDisconnected()
{
    ...
    quint64 sessionId = d->sessionManager->getSessionId(socket);
    if (sessionId == 0) {
...
void WebSocketServer::onBinaryMessageReceived(const QByteArray& message)
{
    ...
    quint64 sessionId = d->sessionManager->getSessionId(socket);
    if (sessionId == 0) {
        LOG_ERROR("Could not find session for socket that sent message");
        return;
    }

    LOG_DEBUG(QString("Received binary message from session %1 (%2 bytes)").arg(sessionId).arg(message.size()));
...
void WebSocketServer::onSocketError(QAbstractSocket::SocketError error)
{
    ...
    if (error == QAbstractSocket::RemoteHostClosedError) {
        LOG_DEBUG(QString("Received RemoteHostClosedError for session %1")
                      .arg(sessionId == 0 ? "unknown" : QString::number(sessionId)));
    } else {
...
bool WebSocketServer::isSessionConnected(quint64 sessionId) const
{
    Q_D(const WebSocketServer);
...
qint64 WebSocketServer::sendBinaryMessage(quint64 sessionId, const QByteArray& data)
{
    Q_D(WebSocketServer);
...
qint64 WebSocketServer::sendMessage(MessagePtr message)
{
    ...
    quint64 sessionId = message->sessionId();
    if (sessionId == 0) {
...
void WebSocketServer::disconnectSession(quint64 sessionId, const QString& reason)
{
    Q_D(WebSocketServer);
