...
{
    if (!m_server || !message) {
        SendResult result(-1, 0, message ? message->sessionId() : QString(), message ? message->typeId() : 0,
    ...
    SendResult result(bytesSent, message->messageId(), message->sessionId(), message->typeId(),
...
void WebSocketServerWorker::disconnectSession(const QString& sessionId, const QString& reason)
{
    if (m_server) {
...
void WebSocketServerWorker::isSessionConnected(const QString& sessionId)
{
...
void WebSocketServerWorker::startServer()
{
    ...
    connect(m_server, &WebSocketServer::sessionConnected, this,
            [this](const QString& sessionId) { emit sessionConnected(sessionId); });
    connect(m_server, &WebSocketServer::sessionDisconnected, this,
            [this](const QString& sessionId, const QString& reason) { emit sessionDisconnected(sessionId, reason); });
...
void ThreadedWebSocketServer::disconnectSession(const QString& sessionId, const QString& reason)
{
    Q_D(ThreadedWebSocketServer);
    if (!d->running.loadAcquire() || !d->serverWorker) {
        return;
    ...
    QMetaObject::invokeMethod(d->serverWorker, "disconnectSession", Qt::QueuedConnection, Q_ARG(QString, sessionId),
                              Q_ARG(QString, reason));
}

void ThreadedWebSocketServer::isSessionConnected(const QString& sessionId)
{
    Q_D(ThreadedWebSocketServer);
    if (!d->running.loadAcquire() || !d->serverWorker) {
        return;
    ...
    QMetaObject::invokeMethod(d->serverWorker, "isSessionConnected", Qt::QueuedConnection, Q_ARG(QString, sessionId));
}
