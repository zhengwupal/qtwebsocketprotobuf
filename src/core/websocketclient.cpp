...
void WebSocketClient::connectToServer(const QUrl& url, quint64 sessionId)
{
    Q_D(WebSocketClient);
    ...
    QUrl connectUrl = url;
    quint64 sessionIdToUse = (sessionId == 0) ? d->savedSessionId : sessionId;
    ...
    if (sessionIdToUse != 0) {
        ...
        query.addQueryItem("session_id", QString::number(sessionIdToUse));
...
SendResult WebSocketClient::sendMessage(MessagePtr message)
{
    ...
    return SendResult(bytesSent, messageId, message->sessionId(), message->messageType(),
...
quint64 WebSocketClient::sessionId() const
{
    Q_D(const WebSocketClient);
    QMutexLocker locker(&d->m_mutex);
    return d->sessionId;
...
void WebSocketClient::setSessionId(quint64 sessionId)
{
    Q_D(WebSocketClient);
    QMutexLocker locker(&d->m_mutex);
